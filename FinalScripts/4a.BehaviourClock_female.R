#### 5.a Behaviour Data - Female - MIXED ####
...
femaleData <- left_join(female_behaviour, BirthPheno_recode, by = "ID")
...
ggplot(LOAOCV_predictions, aes(x = testAgeY, y = testAgeYPRED)) +
  ...
  labs(x = "Age (years)",
       ...
       title = "Behaviour Clock Females (mixed interactions)") +
  xlim(-1, 15) +
  ylim(-1, 15) +
...
#### Writing Outputs ####
# write_csv(fullModelCoef, "OutputData/FinalModelData/BehaviourClock/f_fullModelCoef.csv")

# write_csv(LOAOCV_predictions, "OutputData/FinalModelData/BehaviourClock/f_predictions.csv")
