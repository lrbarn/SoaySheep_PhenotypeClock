#### 7a. Sample clock - female ####
## combining the sheet and behaviour data
...
#### Setting up Data ####
femaleData <- left_join(female_sample_z, BirthPheno_recode)
femaleData$AgeY <- femaleData$CapYear - femaleData$BirthYear

femaleData_all <- na.omit(femaleData)

female_clockData <- femaleData %>% 
  select(ID, AgeY,
         IgA.z, IgE.z, IgG.z,
         RTL.z,
...
variable_names <- c("IgA.z", "IgE.z", "IgG.z",
                    "RTL.z",
                    "Strongyles.z", "Coccidea.z")
...
for (i in 1:n) {
  ...
  TRAIN_glmnet_cv <- cv.glmnet(predictors, response, alpha = set.alpha, type.measure = "mse", standardise = TRUE) 
  
  #4 storing the lambda min for the run
  lambda.min[i] <- TRAIN_glmnet_cv$lambda.min
  
  #5 running again for the lambda min
  TRAIN_glmnet <- glmnet(predictors, response, alpha = set.alpha, nlambda = 100, standardise = FALSE)
...
# fitting the model
glmnet_cv <- cv.glmnet(predictors, response, alpha = set.alpha, type.measure = "mse", standardise = FALSE)

#storing the lambda min 
full_lambda.min <- glmnet_cv$lambda.min

#running again for the lambda min
full_glmnet <- glmnet(predictors, response, alpha = set.alpha, nlambda = 100, standardize = FALSE)
...
#### Writing Outputs ####
# write_csv(fullModelCoef, "OutputData/FinalModelData/BioAssayClock/f_fullModelCoef.csv")
# 
# write_csv(LOAOCV_predictions, "OutputData/FinalModelData/BioAssayClock/f_predictions.csv")
# 
